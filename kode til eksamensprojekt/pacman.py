...
class GhostBehaviour(Enum):
    CHASE = 1
    SCATTER = 2
def translate_screen_to_maze(in_coords, in_size=25):
    return int(in_coords[0] / in_size), int(in_coords[1] / in_size)
def translate_maze_to_screen(in_coords, in_size=25):
...
class GameRenderer:
    ...
    def tick(self, in_fps: int):
        ...
        while not self._done:
            ...
            if self._hero is None: self.display_text("YOU DIED", (self._width / 2 - 240, self._height / 2 - 240), 100)
            if self.get_won(): self.display_text("YOU WON", (self._width / 2 - 275, self._height / 2 - 256), 100)
            pygame.display.flip()
    ...
    def kill_pacman(self):
        self._lives -= 1
        self._hero.set_position(25, 25)
        self._hero.set_direction(Direction.NONE)
        if self._lives == 0: self.end_game()
    def display_text(self, text, in_position=(25, 0), in_size=30):
...
class Hero(MovableObject):
    def __init__(self, in_surface, x, y, in_size: int):
        ...
        self.open = pygame.image.load("sprites/paku.png")
        self.closed = pygame.image.load("sprites/man.png")
...
class Ghost(MovableObject):
    def __init__(self, in_surface, x, y, in_size: int, in_game_controller, sprite_path="sprites/ghost_fright.png"):
        super().__init__(in_surface, x, y, in_size)
        self.game_controller = in_game_controller
        self.sprite_normal = pygame.image.load(sprite_path)
        self.sprite_fright = pygame.image.load("sprites/ghost_fright.png")
    def reached_target(self):
...
class PacmanGameController:
    def __init__(self):
        ...
        self.ghost_colors = [
            "sprites/ghost.png",
            "sprites/Yellowghost.png",
            "sprites/Orangeghost.png",
            "sprites/Redghost.png"
        ]
        self.size = (0, 0)
